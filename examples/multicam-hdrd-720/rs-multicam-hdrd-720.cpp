...
int main(int argc, char * argv[]) try
{
    ...
    for (auto&& dev : ctx.query_devices()) {
        ...
        if (serial == rs_serial)
        {
            ...
            sensor.set_option(RS2_OPTION_EXPOSURE, 15500); // sensor will run at 30fps at 160000. setting exposure to 15500, so sequence 1 will be set to high exposure. 
    ...
    for (auto&& serial : serials)
    {
        if (serial == rs_serial) // RS sensor - depth stream
        {
            ...
            cfg.enable_stream(
                RS2_STREAM_INFRARED, 1, 1280, 720, RS2_FORMAT_Y8, 30);
